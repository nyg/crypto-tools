...
// One ring segment. amount is the holding behind the USD value, shown in the tooltip
...
export function foldSlices(
   slices: Slice[], othersLabel: (count: number) => string = count => `${count} others`
): Slice[] {
...
export default function Donut({ slices, colorFor, emptyText }: {
   ...
   const config: ChartConfig = Object.fromEntries(slices.map(slice =>
      [slice.key, {
                     ...
                     <>
                        ...
                        <div className="flex flex-1 items-center justify-between gap-4 leading-none">
                           ...
                           <span className="font-mono font-medium tabular-nums text-foreground">
                              {asDollarAmount(Number(value))}
                              {item?.payload?.amount !== undefined &&
                                 <span className="ml-2 font-normal text-muted-foreground">
                                    {asAssetAmount(Number(item.payload.amount))} {String(item.payload.key)}
                                 </span>}
                           </span>
                        </div>
                     </>} />} />
