...
// One bar: the period it covers, plus a total per fee type. Every type gets a key in
// every bucket, including zero, or the stack draws nothing.
type ChartRow = { period: string } & Record<string, string | number>
...
function buildChart(byMonth: FeeMonthRow[], asset: string | undefined, granularity: Granularity) {

   const rows = byMonth.filter(row => row.asset === asset)
   if (rows.length === 0) return { data: [] as ChartRow[], types: [] as string[] }
   ...
   for (const row of rows) {
      const bucket = buckets.get(bucketOf(row.month, granularity).key)!
      bucket[row.type] = Number(bucket[row.type]) + row.total
   }
...
export default function FeeChart({
   fees, colors, assets, asset, granularity, onAssetChange, onGranularityChange
}: {
   fees?: FeeSummary
   ...
   const config: ChartConfig = Object.fromEntries(types.map(type =>
