...
export function ledgerSummary(body = {}) {

   const filtered = applyFilters(body.filters)
   const rewardSubTypes = ['allocation', 'deallocation', 'autoallocation', 'migration']

   const byAsset = filtered.reduce((byAsset, entry) => {
      const asset = byAsset[entry.baseAsset] ??=
         { asset: entry.baseAsset, count: 0, net: 0, fees: 0, rewards: 0, balance: null }

      asset.count++
      asset.net += Number(entry.amount) - Number(entry.fee)
      asset.fees += Number(entry.fee)
      if (['staking', 'earn'].includes(entry.type) && !rewardSubTypes.includes(entry.subtype)) {
         asset.rewards += Number(entry.amount) - Number(entry.fee)
      }
      asset.balance = entry.balance

      return byAsset
   }, {})

   const types = filtered.reduce((types, entry) => {
   ...
   return {
      totals: {
         ...
         from: filtered[0]?.time ?? null,
         to: filtered.at(-1)?.time ?? null
      },
      assets: Object.values(byAsset)
         .map(asset => ({
            asset: asset.asset,
            count: asset.count,
            netAmount: asset.net.toFixed(8),
            feeTotal: asset.fees.toFixed(8),
            rewardAmount: asset.rewards.toFixed(8),
