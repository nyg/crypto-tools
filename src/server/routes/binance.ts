...
app.post('/aggregate-balance', async (c) => withCredentials(c, 'binance', async ({ credentials }) => {
   ...
   let spotBalance: SpotBalances
   let stakingPositions: StakingBalances
   let stakingProducts: StakingProducts
   let rates: Record<string, Big | number>
   let assets: string[]
   ...
   const aggregateBalance = assets.map(asset => {
      ...
      // Highest yield first. Sorted into a new array rather than in place: the products
      // come from a map shared across every asset in this response.
      const products = (stakingProducts[asset] ?? [])
         .toSorted((a, b) => Big(b.apy).minus(a.apy).toNumber())
         .map(product => {
            const positions = staking.positions.filter(position => position.id === product.id)
   ...
   aggregateBalance.sort((a, b) => {
      if (a.staking.products.length == 0 && b.staking.products.length != 0) {
         ...
         return b.freeFiatValue.minus(a.freeFiatValue).toNumber()
      }
   })
...
app.post('/balances', async (c) => withCredentials(c, 'binance', async ({ credentials }) => {
   ...
   const balances = assets
      ...
      .reduce<Record<string, Big>>((balances, balance) => {
...
app.post('/deposits', async (c) => withCredentials(c, 'binance', async ({ body, credentials }) => {
   ...
   const computeToDate = (fromDate: number) => fromDate + 90 * 24 * 3600 * 1000
   const delay = (ms: number) => new Promise(r => setTimeout(r, ms))

   let hasNext = true
   let currentFromDate = Number(body.fromDate)
