...
const asDecimalString = (value: string | undefined): string | null => {
...
const asOptionalDecimalString = (value: string | undefined): string => {
   const trimmed = (value ?? '').trim()
...
export default class KrakenAPI {
   ...
   // Every private call needs credentials; a KrakenAPI built without them is only
   // good for the public endpoints, and asking it for a balance is a programming error.
   get #authenticated(): Credentials {
      if (!this.#credentials) throw new Error('This Kraken call needs API credentials.')
   ...
   async createOrders({ orders, ...args }: KrakenOrderBatchParams): Promise<unknown[]> {
      ...
      for (const orderChunk of orderChunks) {
         const response = await resource.createOrderBatch(
            this.#authenticated, { ...args, orders: orderChunk })
   ...
   async fetchPairIndex(): Promise<PairIndex> {
      const assetPairs = (await resource.fetchAllAssetPairs()).result
      return buildPairIndex(assetPairs)
   }

   #pairIndexOnce(): Promise<PairIndex> {
      this.#sharedPairIndex ??= this.fetchPairIndex()
      return this.#sharedPairIndex
   }
   ...
   async fetchOpenOrders(): Promise<OpenOrder[]> {
      ...
      const response = await resource.fetchOpenOrders(this.#authenticated)
      ...
      const pairIndex = await this.#pairIndexOnce()

      return orders
         .map(([txid, order]): OpenOrder => {
   ...
   async fetchPairPrices(rawPairs: string[]): Promise<PairPrices> {

      const wanted = [...new Set(rawPairs.filter(Boolean))]
      if (wanted.length === 0) return {}

      const index = await this.#pairIndexOnce()
   ...
   async cancelOrders(txids: string[]): Promise<CancelResult> {
      ...
      for (const chunk of chunks) {
         const response = await resource.cancelOrderBatch(this.#authenticated, { txids: chunk })
   ...
   async fetchUsdRates(assets: string[]): Promise<UsdRates> {
   ...
   async fetchTokenizedVolumes(): Promise<Map<string, TokenizedVolume>> {
      ...
      const symbols = [...new Set(Object.values(pairs.result ?? {})
         ...
         .filter((wsname): wsname is string => Boolean(wsname)))]
   ...
   async fetchTokenizedListings(): Promise<TokenizedListing[]> {
      ...
      return [...new Map(Object.values(assets)
         ...
         .map((asset): [string, TokenizedListing] => [asset.altname, {
   ...
   async requestExport({ report, description, fromDate, toDate }: ExportRequest): Promise<string> {
      const response = await resource.addExport(
         this.#authenticated, { report, description, fromDate, toDate })
      return response.result.id
   }

   async fetchExportReports(report: ExportReportType = 'ledgers'): Promise<ExportReport[]> {
      const response = await resource.fetchExportStatus(this.#authenticated, report)
   ...
   async removeExport(reportId: string, type = 'delete'): Promise<void> {
      await resource.removeExport(this.#authenticated, { reportId, type })
   }
   ...
   async #readExportRows(reportId: string) {

      const archive = await resource.retrieveExport(this.#authenticated, { reportId })
   ...
   async fetchLedgerEntries(reportId: string): Promise<{ entries: LedgerEntry[], skipped: number }> {

      const rows = await this.#readExportRows(reportId)
   ...
   async fetchTradeEntries(
      reportId: string, pairIndex: PairIndex
   ): Promise<{ trades: Trade[], skipped: number }> {
      ...
      for (const row of rows) {
         ...
         if (!row.txid || Number.isNaN(time)
            || price === null || cost === null || fee === null || vol === null) {
            skipped++
            continue
         }
