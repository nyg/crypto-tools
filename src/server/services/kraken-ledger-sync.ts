...
interface ReportRun<Row> {
   report: ExportReportType
   fromDate: number
   read: (reportId: string) => Promise<{ rows: Row[], skipped: number }>
   count: () => number
   upsert: (chunk: Row[], syncedAt: number) => void
}
...
const reports: ExportReportType[] = ['ledgers', 'trades']
...
export function isRunning(job: SyncJob | null | undefined): boolean {
   return Boolean(job) && !terminalPhases.includes(job!.phase)
}

export function jobFor(accountId: string): SyncJob | null {
   return jobs.get(accountId) ?? null
}

export function requestCancel(accountId: string): SyncJob | null {
   const job = jobs.get(accountId)
   if (job && isRunning(job)) job.cancelRequested = true
   return job ?? null
}

export function startSync(accountId: string, credentials: Credentials, mode: SyncMode = 'incremental'): StartedJob<SyncJob> {
...
function setPhase(job: SyncJob, phase: SyncJob['phase']) {
   job.phase = phase
   job.updatedAt = Date.now()
}

function setStepPhase(job: SyncJob, step: SyncStep, phase: SyncStepPhase) {
   step.phase = phase
   job.updatedAt = Date.now()
}

async function runSync(job: SyncJob, credentials: Credentials) {
   ...
   try {
      ...
      if (job.phase !== 'cancelled') {
         setPhase(job, 'error')
         job.error = String((error as Error)?.cause ?? messageOf(error))
...
async function runReport<Row>(
   job: SyncJob, krakenAPI: KrakenAPI, repository: LedgerRepository,
   { report, fromDate, read, count, upsert }: ReportRun<Row>
) {
...
function startFor(job: SyncJob, repository: LedgerRepository, tradeRepository: TradeRepository, report: ExportReportType): number {
   return job.mode === 'full' ? 0 : incrementalStart(repository, tradeRepository, report)
}

function incrementalStart(repository: LedgerRepository, tradeRepository: TradeRepository, report: ExportReportType): number {
   ...
   const watermark = [coveredTo, range.last].filter((value): value is number => Boolean(value))
...
async function storeRows<Row>(
   job: SyncJob, step: SyncStep, rows: Row[],
   count: () => number, upsert: (chunk: Row[], syncedAt: number) => void
) {
...
function finishLedgerState(repository: LedgerRepository) {
...
function finishTradeState(repository: LedgerRepository, tradeRepository: TradeRepository, startedAt: number) {
...
function finishSyncState(repository: LedgerRepository) {
...
async function removeOrphanedReports(krakenAPI: KrakenAPI) {
   try {
      ...
      console.warn('Could not check for orphaned exports:', messageOf(error))
   }
}

async function removeStepReport(krakenAPI: KrakenAPI, step: SyncStep) {
...
async function safeRemoveReport(krakenAPI: KrakenAPI, reportId: string, type = 'delete') {
   try {
      await krakenAPI.removeExport(reportId, type)
   }
   catch (error) {
      // Never let a failed cleanup mask the error that actually ended the run.
      console.warn('Could not remove Kraken export', reportId, messageOf(error))
   }
}
