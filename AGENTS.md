...
**Adapters** (`src/server/adapters/`) — each external API has an adapter directory (`binance-api/`, `binance-gateway-api/`, `bybit-api/`, `frankfurter-api/`, `kraken-api/`, `anthropic/`) following a three-layer pattern:
...
`frankfurter-api/` reads the ECB's daily reference rates from api.frankfurter.dev, a public API with no key, so it has no authenticator.
...
**Services** (`src/server/services/`) — `rate-finder.ts` uses Dijkstra's algorithm (`modern-dijkstra`) to find trading pair paths and calculate fiat rates against USDT. `kraken-ledger-sync.ts` runs the multi-step ledger export as a background job held in an in-memory registry keyed by account, which the page follows by polling a status endpoint. Its third step, `usd-rate-backfill.ts`, stores the daily USD rates the Rewards and Fees pages value the ledger at, for every asset with a reward or a fee, and only the days it does not have yet. Crypto comes from Kraken's `OHLC`: the daily VWAP, which Kraken only serves for the last 720 days whatever `since` asks for, and the weekly one spread over its seven days before that. Fiat (EUR, GBP, CHF, CAD, JPY, AUD) comes from the ECB through Frankfurter, daily back to 1999, in one call. An asset Kraken renamed and whose old pair it no longer serves takes the days before its new pair from Binance's public daily klines of the old ticker, listed in `LEGACY_USD_SYMBOLS` (POL from `MATICUSDT`, since Kraken answers `EQuery:Invalid asset pair` for `MATICUSD`); those rows are inserted, never upserted, so a Kraken rate always wins. The current UTC day is never stored, because its candle is not over. A failed rate step leaves the run `done`: the ledger and trades are already stored, and the next sync fills the gap. The Rewards and Fees routes also call `refreshUsdRates`, once per server process and account, so rates missing after an upgrade or a failed step are fetched in the background without an export sync; the response says `ratesPending` until it is done, and the page polls until then.
