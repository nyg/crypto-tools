...
- **Open Orders** — Everything still on Kraken's book, read live from the exchange and grouped by trading pair. Each table shows ten orders at a time and foots with the total and the per-order average of volume, price and cost; a pair holding both buys and sells can be switched between the two. Orders carry the reference they were created with, the time they were opened in your own timezone, and how far the market has to move for each to fill. Cancel a single order, or select several and cancel them in one call.
